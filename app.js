...
function importDataJsFile() {
    ...
    if (!fileInput || !fileInput.files.length) {
        showSnackbar("⚠️ Sélectionnez un fichier");
        return;
    }
    ...
    reader.onload = function(e) {
        ...
        // On crée une fonction qui retourne la variable 'agents' du fichier
        let importedAgents = null;
        try {
            // Méthode : on exécute le code dans une fonction anonyme
        ...
        saveData();
        closePopup();
        displayAgentsList(); // rafraîchit la liste
    };
    reader.onerror = function() {
        showSnackbar("❌ Erreur de lecture du fichier");
    };
    reader.readAsText(file, 'UTF-8');
}
